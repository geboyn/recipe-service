...
public enum RecipeType {
    ...
    public static RecipeType getValue(String value) {
        return Arrays.stream(RecipeType.values()).filter(aEnum -> aEnum.name().equalsIgnoreCase(value)).findFirst().orElseThrow(() ->
                new InvalidRecipeException(INVALID_RECIPE_TYPE.getErrorCode(), INVALID_RECIPE_TYPE.getMessage())
        );
    }
}
